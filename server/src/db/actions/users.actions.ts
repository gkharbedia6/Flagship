...
export const createUnvefiriedUser = (values: Record<string, any>) =>
  new UnverifiedUserModel(values).save().then((user) => user.toObject());

export const createForgotPasswordUser = (values: Record<string, any>) =>
  new ForgotPasswordUserModel(values).save().then((user) => user.toObject());
...
export const updateForgotPasswordUserById = (
  values: Record<string, any>,
  id: string
) => ForgotPasswordUserModel.findByIdAndUpdate(id, values, { new: true });
...
export const deleteForgotPasswordUserById = (id: string) =>
  ForgotPasswordUserModel.findOneAndDelete({ _id: id });
